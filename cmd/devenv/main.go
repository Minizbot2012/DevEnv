package main

import (
	"encoding/json"
	"os"
	"os/exec"
	"path"
	"runtime"
	"strings"
)

type vars struct {
	Name  string
	Value string
}

type Env struct {
	//add to environment
	Environ []vars `json:"vars"`
	//append to PATH
	PathAppend []string `json:"path"`
	//Vars to keep and pass
	Passthrough []string `json:"keep"`
	//Terminal to use
	Terminal string
}

//$(PWD) -> getwd
//$(OS) -> runtime.GOOS
//$(ARCH) -> runtime.GOARCH
func replacements(str string) (ret string) {
	cwd, _ := os.Getwd()
	ret = str
	ret = strings.ReplaceAll(ret, `$(PWD)`, cwd)
	ret = strings.ReplaceAll(ret, `$(OS)`, runtime.GOOS)
	ret = strings.ReplaceAll(ret, `$(ARCH)`, runtime.GOARCH)
	return ret
}

func main() {
	//Init config file
	cwd, _ := os.Getwd()
	jsf, err := os.Open(path.Join(cwd, "config_"+runtime.GOOS+".json"))
	if err != nil {
		jsf, err = os.Open(path.Join(cwd, "config.json"))
		if err != nil {
			panic(err)
		}
	}
	defer jsf.Close()
	var environ Env
	//Decode config file
	err = json.NewDecoder(jsf).Decode(&environ)
	if err != nil {
		panic(err)
	}
	// Default passthrough
	environ.Passthrough = append(environ.Passthrough, "SSH_AUTH_SOCK")
	if runtime.GOOS == "windows" {
		environ.Passthrough = append(environ.Passthrough,
			"PROCESSOR_ARCHITECTURE", "PROCESSOR_IDENTIFIER",
			"PSModulePath", "PATHEXT", "OS", "DriverData", "NUMBER_OF_PROCESSORS", "USERNAME",
			"APPDATA", "PROGRAMFILES", "ProgramFiles(x86)", "ProgramData")
	} else if runtime.GOOS == "linux" {
		environ.Passthrough = append(environ.Passthrough,
			"DISPLAY", "WAYLAND_DISPLAY", "LANG", "HOME", "XDG_RUNTIME_DIR",
			"XDG_SESSION_TYPE", "XDG_SESSION_DESKTOP", "USER", "SHELL", "SESSION_MANAGER",
			"TERM", "XAUTHORITY", "DESKTOP_SESSION", "HISTCONTROL", "COLORTERM")
	}
	//do our replacements
	//$PWD

	for i, v := range environ.Environ {
		environ.Environ[i].Value = strings.ReplaceAll(v.Value, `$(PWD)`, cwd)
	}
	for i, v := range environ.PathAppend {
		environ.PathAppend[i] = strings.ReplaceAll(v, `$(PWD)`, cwd)
	}

	//Initiate Terminal
	term, err := exec.LookPath(environ.Terminal)
	if err != nil {
		panic(err)
	}

	cmd := exec.Command(term)
	cmd.Env = []string{}
	path := os.Getenv("PATH")

	//Path Appends
	for _, v := range environ.PathAppend {
		path = path + v + string(os.PathListSeparator)
	}
	cmd.Env = append(cmd.Env, "PATH="+path)

	//Environment Variables
	for _, v := range environ.Environ {
		cmd.Env = append(cmd.Env, v.Name+"="+v.Value)
	}

	//Passthrough Variables
	for _, v := range environ.Passthrough {
		cmd.Env = append(cmd.Env, v+"="+os.Getenv(v))
	}

	cmd.Stdin = os.Stdin
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	cmd.Run()
}
